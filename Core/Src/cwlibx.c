...
void led_t_initializer(){
	leds[0].porta = LED_BC1_GPIO_Port;
	leds[0].pin = 	LED_BC1_Pin;
	leds[0].state = 0;

	leds[1].porta = LED_BC2_GPIO_Port;
	leds[1].pin = 	LED_BC2_Pin;
	leds[1].state = 0;

	leds[2].porta = LED_BC3_GPIO_Port;
	leds[2].pin = 	LED_BC3_Pin;
	leds[2].state = 0;

	leds[3].porta = LED_BC4_GPIO_Port;
	leds[3].pin = 	LED_BC4_Pin;
	leds[3].state = 0;

	leds[4].porta = LED_BC5_GPIO_Port;
	leds[4].pin = 	LED_BC5_Pin;
	leds[4].state = 0;

	leds[5].porta = LED_BC6_GPIO_Port;
	leds[5].pin = 	LED_BC6_Pin;
	leds[5].state = 0;

	leds[6].porta = LED_BC7_GPIO_Port;
	leds[6].pin = 	LED_BC7_Pin;
	leds[6].state = 0;

	leds[7].porta = LED_BC8_GPIO_Port;
	leds[7].pin = 	LED_BC8_Pin;
	leds[7].state = 0;
}
...
*/
/**@brief	Baterry level read
 * Esta função lê o nivel da bateria e envia para a porta usb
 *
 *
 */
void battery_lvl_read(void){
	char msg[10];
	uint16_t raw;
	//habilitando a leitura
	HAL_GPIO_WritePin(EN_BATLV_GPIO_Port, EN_BATLV_Pin, GPIO_PIN_SET);
	//fazendo a leitura
	HAL_ADC_Start(&hadc1);
	HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
	raw = HAL_ADC_GetValue(&hadc1);
	//desabilita a leitura
	HAL_GPIO_WritePin(EN_BATLV_GPIO_Port, EN_BATLV_Pin, GPIO_PIN_RESET);
	//convertendo
	ddp[0] = (6*raw)/1000;
	/*if(*/ddp[1] = (((6*raw)/100)-ddp[0]); /*> 5){
		//correção no arredondamento graças ao calculo estar sendo exclusivamente com inteiros
		converted++;
	}*/
	//trannsmitinto o valor lido
	snprintf(msg, sizeof(msg), "%d,%d", ddp[0],ddp[1]);
	monitor_send_string(msg);
}
...
//		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
		...
		//__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, LCD_BRIGHT);
